...
class Labeling(ABC):
    ...
    @abstractmethod
    def fit_predict_cluster(self, docs: List[np.ndarray]) -> str:
        raise NotImplementedError

    def fit_predict(self, docs: List[np.ndarray], clusters: np.ndarray) -> List[str]:
...
class DummyLabeling(Labeling):
    ...
    def fit_predict_cluster(self, docs: List[np.ndarray], clusters: np.ndarray) -> str:
        return ""
